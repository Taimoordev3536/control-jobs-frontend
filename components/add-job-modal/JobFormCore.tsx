...
export default function JobFormCore({ jobId, mode, onComplete, onCancel }: JobFormCoreProps) {
    ...
    if (mode === "edit" && jobId) {
      const fetchJobData = async () => {
        ...
        try {
          ...
          console.log('===== RAW JOB DATA FROM BACKEND =====')
          console.log('Full job object:', JSON.stringify(job, null, 2))
          console.log('jobName:', job.jobName)
          console.log('scheduleType:', job.scheduleType)
          console.log('seasonalSchedules:', JSON.stringify(job.seasonalSchedules, null, 2))
          console.log('customerSurvey:', JSON.stringify(job.customerSurvey, null, 2))
          console.log('workerSurvey:', JSON.stringify(job.workerSurvey, null, 2))
          console.log('tasks:', job.tasks?.length || 0, 'tasks')
          console.log('====================================')
          ...
          setFormData((prev) => ({
            ...
            workCenterIds: job.workCenterIds?.map((id: any) => String(id)) || [],
            workerIds: job.workerIds?.map((id: any) => String(id)) || [],
            observations: job.note || "",
            scheduleType: job.scheduleType || "free",
            ...
            schedules: (() => {
              ...
              if (job.seasonalSchedules && Array.isArray(job.seasonalSchedules)) {
                job.seasonalSchedules.forEach((ss: any) => {
                  ...
                  if (ss.shifts && Array.isArray(ss.shifts)) {
                    ss.shifts.forEach((shift: any) => {
                      ...
                      const startTime = shift.baseStartTime || ""
                      const endTime = shift.baseEndTime || ""
                      
                      const daysOfWeek = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
                      
                      // Determine shift period based on start time hour
                      let shiftPeriod: ShiftKey = "morning"
                      if (startTime) {
                        const hour = parseInt(startTime.split(":")[0])
                        if (hour >= 12 && hour < 17) shiftPeriod = "afternoon"
                        else if (hour >= 17) shiftPeriod = "evening"
                      }
                      
                      // Check if it's a single day shift or multi-day shift
                      if (startDay === endDay) {
                        // Single day shift: put both start and end time on the same day
                        if (schedules[season][startDay as keyof typeof schedules.normal]) {
                          schedules[season][startDay as keyof typeof schedules.normal][shiftPeriod] = {
                            start: startTime,
                            end: endTime
                          }
                          
                          // Calculate total hours for single day shift
                          if (startTime && endTime) {
                            ...
                            schedules[season][startDay as keyof typeof schedules.normal].total = minutesToTime(totalMinutes)
                          }
                        }
                      } else {
                        ...
                        if (startIdx !== -1 && endIdx !== -1) {
                          ...
                          if (schedules[season][startDay as keyof typeof schedules.normal]) {
                            const hoursOnStartDay = (24 * 60 - startMinutes) / 60
                            schedules[season][startDay as keyof typeof schedules.normal][shiftPeriod] = {
                              start: startTime,
                              end: ""
                            }
                            schedules[season][startDay as keyof typeof schedules.normal].total = minutesToTime(Math.floor(hoursOnStartDay * 60))
                          }
                          ...
                          while (currentIdx < endIdx) {
                            ...
                            if (schedules[season][dayKey as keyof typeof schedules.normal]) {
                              schedules[season][dayKey as keyof typeof schedules.normal].total = "24:00"
                            }
                            currentIdx++
                          }
                          
                          // End day: from midnight to end time
                          if (schedules[season][endDay as keyof typeof schedules.normal]) {
                            const hoursOnEndDay = endMinutes / 60
                            schedules[season][endDay as keyof typeof schedules.normal][shiftPeriod] = {
                              start: "",
                              end: endTime
                            }
                            schedules[season][endDay as keyof typeof schedules.normal].total = minutesToTime(Math.floor(hoursOnEndDay * 60))
                          }
                        }
                      }
                    })
            ...
            seasonalSchedules: job.seasonalSchedules?.map((ss: any) => ({
              season: ss.season,
              startDate: ss.startDate || "",
              endDate: ss.endDate || "",
              totalWeekHours: ss.totalWeekHours || 0,
              shifts: ss.shifts?.map((shift: any) => ({
            ...
            seasonPeriods: job.seasonalSchedules?.filter((ss: any) => ss.startDate || ss.endDate).map((ss: any) => ({
              season: ss.season,
            ...
            signingMethods: (() => {
              ...
              if (job.signingMethods && Array.isArray(job.signingMethods)) {
                job.signingMethods.forEach((sm: any) => {
                  const deviceType = sm.methodType === "pc" ? "laptop" : sm.methodType
                  if (deviceType && methods[deviceType as keyof typeof methods]) {
                    const details = sm.methodDetails || []
                    details.forEach((detail: string) => {
                      const key = detail.toLowerCase()
                      // Map backend field names to frontend field names
                      if (key === "web") {
                        methods[deviceType as keyof typeof methods].wifi = true
                      } else if (key === "qrcode") {
            ...
            entrance: (() => {
              const entryAlert = job.alerts?.find((a: any) => a.alertType === 'sign_in')
              return {
                whenSigningIn: !!entryAlert,
                delay: entryAlert?.triggerTime !== null && entryAlert?.triggerTime !== undefined,
                delayValue: entryAlert?.triggerTime ? String(entryAlert.triggerTime) : "15"
              }
            })(),
            
            exit: (() => {
              const exitAlert = job.alerts?.find((a: any) => a.alertType === 'sign_out')
              return {
                whenSigningIn: !!exitAlert,
                duration: exitAlert?.minDuration !== null && exitAlert?.minDuration !== undefined,
                durationValue: exitAlert?.minDuration ? String(exitAlert.minDuration) : "30"
              }
            })(),
            ...
            customerSurvey: job.customerSurvey ? {
              ...
              monitoringValue: job.customerSurvey.rateDigit ? [job.customerSurvey.rateDigit] : [5],
              ...
              hour: job.customerSurvey.sendTime || "08:00",
              interval: job.customerSurvey.interval || 1,
              weeklyDays: (() => {
                try {
                  const data = job.customerSurvey.weeklyDays
                  if (!data) return []
                  if (Array.isArray(data)) return data.map((d: any) => typeof d === 'string' ? parseInt(d) : d)
                  if (typeof data === 'string') return JSON.parse(data).map((d: any) => typeof d === 'string' ? parseInt(d) : d)
                  return []
                } catch { return [] }
              })(),
              monthlyDays: (() => {
              ...
              monthlyMode: (() => {
                ...
                const hasMonthlyDays = Array.isArray(monthlyDays) ? monthlyDays.length > 0 : false
                const hasMonthlyWeekdays = Array.isArray(monthlyWeekdays) ? monthlyWeekdays.length > 0 : false
                if (job.customerSurvey.monthlyStartWeekday !== null && job.customerSurvey.monthlyStartWeekday !== undefined) return "firstWeekDay"
                if (job.customerSurvey.monthlyEndWeekday !== null && job.customerSurvey.monthlyEndWeekday !== undefined) return "lastWeekDay"
                if (hasMonthlyWeekdays) return "weekdays"
                return "dates"
              })(),
              monthlyFirstWeekday: job.customerSurvey.monthlyStartWeekday !== undefined && job.customerSurvey.monthlyStartWeekday !== null 
                ? (typeof job.customerSurvey.monthlyStartWeekday === 'string' ? parseInt(job.customerSurvey.monthlyStartWeekday) : job.customerSurvey.monthlyStartWeekday)
                : null,
              monthlyLastWeekday: job.customerSurvey.monthlyEndWeekday !== undefined && job.customerSurvey.monthlyEndWeekday !== null
                ? (typeof job.customerSurvey.monthlyEndWeekday === 'string' ? parseInt(job.customerSurvey.monthlyEndWeekday) : job.customerSurvey.monthlyEndWeekday)
                : null,
            } : {
              questionText: "",
              monitoringValue: [5],
            ...
            workerSurvey: job.workerSurvey ? {
              ...
              monitoringValue: job.workerSurvey.rateDigit ? [job.workerSurvey.rateDigit] : [5],
              ...
              hour: job.workerSurvey.sendTime || "08:00",
              interval: job.workerSurvey.interval || 1,
              weeklyDays: (() => {
                console.log("🔍 WORKER SURVEY weeklyDays RAW:", job.workerSurvey.weeklyDays)
                try {
              ...
              monthlyMode: (() => {
                ...
                const hasMonthlyDays = Array.isArray(monthlyDays) ? monthlyDays.length > 0 : false
                const hasMonthlyWeekdays = Array.isArray(monthlyWeekdays) ? monthlyWeekdays.length > 0 : false
                console.log("🔍 WORKER SURVEY monthlyMode detection:", { monthlyDays, monthlyWeekdays, hasMonthlyDays, hasMonthlyWeekdays, monthlyStartWeekday: job.workerSurvey.monthlyStartWeekday, monthlyEndWeekday: job.workerSurvey.monthlyEndWeekday })
                if (job.workerSurvey.monthlyStartWeekday !== null && job.workerSurvey.monthlyStartWeekday !== undefined) return "firstWeekDay"
                if (job.workerSurvey.monthlyEndWeekday !== null && job.workerSurvey.monthlyEndWeekday !== undefined) return "lastWeekDay"
                if (hasMonthlyWeekdays) return "weekdays"
                return "dates"
              })(),
              monthlyFirstWeekday: job.workerSurvey.monthlyStartWeekday !== undefined && job.workerSurvey.monthlyStartWeekday !== null
                ? (typeof job.workerSurvey.monthlyStartWeekday === 'string' ? parseInt(job.workerSurvey.monthlyStartWeekday) : job.workerSurvey.monthlyStartWeekday)
                : null,
              monthlyLastWeekday: job.workerSurvey.monthlyEndWeekday !== undefined && job.workerSurvey.monthlyEndWeekday !== null
                ? (typeof job.workerSurvey.monthlyEndWeekday === 'string' ? parseInt(job.workerSurvey.monthlyEndWeekday) : job.workerSurvey.monthlyEndWeekday)
                : null,
            } : {
              questionText: "",
              monitoringValue: [5],
            ...
            tasks: job.tasks?.map((task: any) => ({
              ...
              duration: task.expectedDuration ? String(task.expectedDuration) : "",
              shifts: task.shift ? { [task.shift]: true } : { morning: false, afternoon: false, evening: false },
              ...
              monthlyMode: task.monthlyDays?.length > 0 ? "dates" : (task.monthlyWeekdays?.length > 0 ? "weekdays" : "dates"),
              monthlyFirstWeekday: task.monthlyStartWeekday !== undefined && task.monthlyStartWeekday !== null ? task.monthlyStartWeekday : null,
              monthlyLastWeekday: task.monthlyEndWeekday !== undefined && task.monthlyEndWeekday !== null ? task.monthlyEndWeekday : null,
          ...
          // Enable surveys if either has actual data (questionText is a good indicator)
          if (
            (job.customerSurvey && job.customerSurvey.questionText) || 
            (job.workerSurvey && job.workerSurvey.questionText)
          ) {
            setEnableSurveys(true)
          }

          toast({
            title: t("jobLoaded") || "Job loaded",
            description: t("jobDataLoadedSuccessfully") || "Job data loaded successfully",
          })
        } catch (error) {
          console.error("Error fetching job:", error)
          toast({
            title: t("error") || "Error",
            description: t("failedToLoadJob") || "Failed to load job data",
            variant: "destructive",
          })
        } finally {
          setIsLoading(false)
  ...
  const addTaskToList = () => {
    ...
    if (editingTaskId) {
      ...
      setFormData((prev) => ({
        ...prev,
        tasks: prev.tasks.map((t) => (t.id === editingTaskId ? { ...taskData, id: t.id } : t)),
        task: "",
        taskObservations: "",
        taskWorkCenterId: "",
        duration: "",
      ...
      toast({
        ...
        description: t("taskUpdatedDescription") || "Task has been updated successfully",
      })
    } else {
      // Add new task
      ...
      toast({
        ...
        description: t("taskAddedDescription") || "Task has been added to the list",
      })
    }
  }
  ...
  const editTask = (taskId: string) => {
    ...
    setFormData((prev) => ({
      ...
      taskWorkCenterId: taskToEdit.workCenterId !== null && taskToEdit.workCenterId !== undefined 
        ? String(taskToEdit.workCenterId) 
        : "",
      duration: taskToEdit.duration || "",
      shifts: { ...taskToEdit.shifts },
      toBeCarriedOut: taskToEdit.toBeCarriedOut,
      ...
      weeklyDays: (taskToEdit.weeklyDays || []).map((d: any) => typeof d === 'string' ? parseInt(d) : d),
      monthlyDays: (taskToEdit.monthlyDays || []).map((d: any) => typeof d === 'string' ? parseInt(d) : d),
      monthlyWeekdays: (taskToEdit.monthlyWeekdays || []).map((d: any) => typeof d === 'string' ? parseInt(d) : d),
      monthlyMode: taskToEdit.monthlyMode || "dates",
      monthlyFirstWeekday: taskToEdit.monthlyFirstWeekday !== null && taskToEdit.monthlyFirstWeekday !== undefined
        ? (typeof taskToEdit.monthlyFirstWeekday === 'string' ? parseInt(taskToEdit.monthlyFirstWeekday) : taskToEdit.monthlyFirstWeekday)
        : null,
      monthlyLastWeekday: taskToEdit.monthlyLastWeekday !== null && taskToEdit.monthlyLastWeekday !== undefined
        ? (typeof taskToEdit.monthlyLastWeekday === 'string' ? parseInt(taskToEdit.monthlyLastWeekday) : taskToEdit.monthlyLastWeekday)
        : null,
      yearlyMonths: (taskToEdit.yearlyMonths || []).map((m: any) => typeof m === 'string' ? parseInt(m) : m),
      yearlyDays: (taskToEdit.yearlyDays || []).map((d: any) => typeof d === 'string' ? parseInt(d) : d),
      alertTaskCompleted: taskToEdit.alertTaskCompleted || false,
      pendingTaskAlert: taskToEdit.pendingTaskAlert || false,
    }))
    setEditingTaskId(taskId)

    console.log("🔍 AFTER SETTING - taskWorkCenterId in prev state:", formData.taskWorkCenterId)
    console.log("🔍 AFTER SETTING - taskStartDate in prev state:", formData.taskStartDate)
    ...
    if (currentMainStep === 1) {
      ...
      if (mode === "create") {
        ...
        if (startDateObj < today) {
          setErrors({ startDate: t("startDateCannotBeInPast") || "Start date cannot be in the past" })
          return
        }
      }
      ...
      if (formData.scheduleType === "programming") {
        ...
        const buildShiftBlocks = (schedules: any) => {
          ...
          for (const sEv of starts) {
            let chosen: { end: any; endIndex: number; adjustedDayIndex: number } | null = null
        ...
        console.log("🔍 DEBUG - formData.seasonPeriods:", JSON.stringify(formData.seasonPeriods, null, 2))
        ...
        ;(["normal", "summer"] as const).forEach((season) => {
          ...
          console.log(`🔍 DEBUG - ${season} seasonPeriod:`, JSON.stringify(seasonPeriod, null, 2))
          ...
          console.log(`🔍 DEBUG - ${season} seasonObj:`, JSON.stringify(seasonObj, null, 2))
      ...
      if (enableTasks && formData.tasks.length > 0) {
        formData.tasks.forEach((task) => {
          ...
          tasks.push({
            ...
            expectedDuration: task.duration ? Number.parseInt(task.duration) : 0,
  ...
  }, [formData, enableTasks, enableSurveys, session, t, mode, jobId, onComplete, resetForm])
