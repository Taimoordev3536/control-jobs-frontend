...
export function ClientFilesTab({ clientId }: ClientFilesTabProps) {
  ...
  const onPick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/client/${clientId}/files`, { method: "POST", headers: authHeader, body: fd })
      if (!res.ok) {
  ...
  const remove = async (id: string) => {
    ...
    try {
      await fetch(`${base}/client/${clientId}/files/${id}`, { method: "DELETE", headers: authHeader })
      ...
      <div className="flex flex-wrap items-end gap-3 border-b border-border pb-4">
        <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
          ...
          <Input
            ...
            placeholder={t("fileDescriptionPlaceholder") || "e.g. ID card, report…"}
            className="h-9"
          />
        </div>
        <input
